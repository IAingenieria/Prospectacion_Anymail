...
class ApifyScraper:
    ...
    async def check_credits(self) -> dict:
        """
        Verifica que la cuenta Apify tiene presupuesto disponible.
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 401:
                    return {"ok": False, "error": "Token de Apify inválido"}
                if resp.status_code == 403:
                    return {"ok": False, "error": "Sin créditos o límite de gasto alcanzado"}
                if resp.status_code != 200:
                    return {"ok": False, "error": f"Apify HTTP {resp.status_code}"}

                data = resp.json().get("data", {})
                cycle = data.get("monthlyUsageCycle", {})
                limit = cycle.get("monthlyBasePlatformUsageUsd", 0)
                used = cycle.get("usedBasePlatformUsageUsd", 0)

                if limit > 0 and used >= limit * 0.95:
                    return {
                        "ok": False,
                        "error": f"Créditos casi agotados (${used:.2f} / ${limit:.2f} USD)",
                    }
                return {"ok": True, "details": {"used_usd": used, "limit_usd": limit}}

        except Exception as e:
            # Si no podemos verificar, continuamos y dejamos que el run falle si no hay créditos
            logger.warning(f"No se pudo verificar créditos Apify: {e} — continuando de todas formas")
    ...
    async def _run_actor(self, input_data: dict) -> Optional[str]:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code == 403:
                logger.warning(
                    "Apify rechazó el run (403) — límite de gasto alcanzado. "
                    "Revisar en console.apify.com → Billing → Spending limits"
                )
                return None  # El pipeline continuará sin Apify
