...
# ── Códigos de municipio para Nuevo León (para búsquedas específicas) ────────
MUNICIPIOS_NL: dict[str, str] = {
    "monterrey": "039",
    "guadalupe": "026",
    "santa catarina": "048",
    "santacatarina": "048",
    ...
    "apodaca": "006",
    "cadereyta": "009",
    "cadereyta jimenez": "009",
    "cadereyta jiménez": "009",
    "caderayta": "009",
    "san nicolas": "046",
    "san nicolás": "046",
    "san nicolas de los garza": "046",
    "garcia": "018",
    "san pedro": "019",
    "garza garcia": "019",
    "san pedro garza garcia": "019",
    "juarez": "031",
    "juárez": "031",
    "santiago": "049",
    "linares": "033",
...
SECTORES_DENUE: dict[str, dict] = {
    ...
    "parques_industriales": {
        ...
        "scian_sector": "53", "scian_subsector": "531", "scian_rama": "0",
        "terminos": ["parque industrial", "zona industrial", "corredor industrial",
                     "administradora parque industrial", "desarrolladora industrial",
                     "fraccionamiento industrial", "ciudad industrial"],
        "categoria_default": "Parque Industrial",
        "descripcion": "parques industriales, zonas industriales, corredores industriales",
    },
    "manufactura": {
        "label": "Manufactura / Industria",
        "scian_sector": "31", "scian_subsector": "0", "scian_rama": "0",
        "terminos": ["manufactura", "industria", "planta industrial",
                     "maquiladora", "fabricacion", "ensamble"],
        "categoria_default": "Manufactura / Industria",
        "descripcion": "manufactura, maquiladoras, plantas industriales, ensamble",
    },
    "quimica": {
        "label": "Empresas Químicas / Laboratorios",
        "scian_sector": "32", "scian_subsector": "325", "scian_rama": "0",
        "terminos": ["quimica", "laboratorio", "productos quimicos",
                     "limpieza industrial", "desengrasante", "biodegradable",
                     "solvente", "detergente industrial"],
        "categoria_default": "Empresa Química / Laboratorio",
        "descripcion": "empresas químicas, laboratorios, productos de limpieza industrial",
    },
    "abogados": {
        "label": "Despachos de Abogados / Servicios Legales",
        "scian_sector": "54", "scian_subsector": "541", "scian_rama": "5411",
        "terminos": ["abogado", "despacho abogados", "bufete juridico",
                     "notaria", "asesoria legal", "firma legal",
                     "consultor juridico", "litigante"],
        "categoria_default": "Despacho de Abogados",
        "descripcion": "despachos de abogados, bufetes jurídicos, notarías, asesoría legal",
    },
}
...
class DenueEnricher:
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token:
            raise ValueError(
                ...
                "DENUE_API_KEY=4e4956b1-96b2-408c-a52f-d2c9412ec5a4"
            )
    ...
    async def ingestar_sector(
        ...
        # Determinar códigos de municipio a recorrer
        if municipios:
            muni_codes = []
            for m in municipios:
                code = MUNICIPIOS_NL.get(m.lower().strip())
                if code:
                    muni_codes.append(code)
            if not muni_codes:
                muni_codes = [municipio]
        else:
            muni_codes = [municipio]
        ...
        for muni_code in muni_codes:
            ...
            if cfg_sector["scian_sector"] != "0":
                ...
                todos.extend(por_scian)
                logger.info(
                    f"  SCIAN {cfg_sector['scian_sector']} muni={muni_code}: "
            ...
            for termino in cfg_sector["terminos"]:
                if muni_code != "0":
                    # Filtrar por municipio usando BuscarAreaAct con nombre
                    lote = await self.buscar_por_actividad_scian(
                        entidad=entidad,
                        municipio=muni_code,
                        nombre=termino,
                        start=1,
                    ...
                    # Sin filtro de municipio — búsqueda estatal completa
                    lote = await self.buscar_por_estado(termino, entidad, start=1, end=100)
                todos.extend(lote)
                logger.info(f"  '{termino}' muni={muni_code}: {len(lote)} resultados")
